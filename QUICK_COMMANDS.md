# 🚀 Commandes Rapides - Publication dnd-5e-core

## ⚡ Copier-Coller Direct

### 🏗️ Build du Package
```bash
cd /Users/display/PycharmProjects/dnd-5e-core
rm -rf dist/ build/ *.egg-info/
pip install --upgrade build twine
python -m build
twine check dist/*
```

### 📦 Publication PyPI (Production)
```bash
twine upload dist/*
```

### 🧪 Publication TestPyPI (Test d'abord)
```bash
twine upload --repository testpypi dist/*
```

### 🐙 GitHub Tag & Release
```bash
git tag -a v0.1.1 -m "Version 0.1.1 - Complete D&D 5e Rules Engine"
git push origin v0.1.1
```

### ✅ Test d'Installation Locale
```bash
python -m venv test-env
source test-env/bin/activate
pip install dist/dnd_5e_core-0.1.1-py3-none-any.whl
python -c "from dnd_5e_core.data import load_monster; print(load_monster('goblin')['name'])"
deactivate
rm -rf test-env/
```

### 🧹 Nettoyage
```bash
rm -rf dist/ build/ *.egg-info/ __pycache__/ .pytest_cache/
```

---

## 📋 Configuration Initiale (Une seule fois)

### Créer ~/.pypirc
```bash
cat > ~/.pypirc << 'EOF'
[pypi]
username = __token__
password = pypi-VOTRE_TOKEN_ICI

[testpypi]
username = __token__
password = pypi-VOTRE_TOKEN_TESTPYPI_ICI
EOF

chmod 600 ~/.pypirc
```

### Obtenir les Tokens
- PyPI: https://pypi.org/manage/account/token/
- TestPyPI: https://test.pypi.org/manage/account/token/

---

## 🔍 Commandes de Vérification

### Vérifier la version
```bash
grep "version = " pyproject.toml
```

### Lister les fichiers du package
```bash
tar -tzf dist/dnd-5e-core-0.1.1.tar.gz | head -20
```

### Voir les métadonnées
```bash
python -m build && tar -xzf dist/dnd-5e-core-0.1.1.tar.gz -O dnd-5e-core-0.1.1/PKG-INFO
```

### Compter les monstres
```bash
python -c "from dnd_5e_core.data import list_monsters; print(len(list_monsters()))"
```

---

## 📊 URLs Importantes

### Après Publication
- **PyPI:** https://pypi.org/project/dnd-5e-core/
- **GitHub:** https://github.com/codingame-team/dnd-5e-core
- **Releases:** https://github.com/codingame-team/dnd-5e-core/releases

### Pour Configuration
- **PyPI Account:** https://pypi.org/account/
- **PyPI Tokens:** https://pypi.org/manage/account/token/
- **TestPyPI:** https://test.pypi.org/
- **GitHub Releases:** https://github.com/codingame-team/dnd-5e-core/releases/new

---

## 🎯 Workflow Complet en Une Commande

```bash
cd /Users/display/PycharmProjects/dnd-5e-core && \
rm -rf dist/ build/ *.egg-info/ && \
python -m build && \
twine check dist/* && \
echo "✅ Build terminé! Maintenant: twine upload dist/*"
```

---

## 📚 Guides Détaillés

Pour plus d'informations, consultez:
- `PUBLICATION_CHECKLIST.md` - Checklist complète
- `PUBLICATION_EXPLAINED.md` - Explications détaillées
- `SUMMARY_SOLUTIONS.md` - Réponses à vos questions

---

**Dernière mise à jour:** 5 janvier 2026

